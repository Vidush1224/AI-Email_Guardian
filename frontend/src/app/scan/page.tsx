...
export default function ScanPage() {
  const [form, setForm] = useState({ sender: 'suspicious@external.com', recipients: 'you@company.com', cc: '', subject: '', body: '', direction: 'INBOUND' });
  ...
  const analyze = useCallback((data: typeof form) => {
    ...
    if (data.sender && !data.sender.includes('@company.com')) { sigs.push('external_domain'); s += 15; f.domain_risk = 40; }
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      // If backend is unreachable, explicitly run the client-side analysis analyzer
      analyze(form);
    }
  ...
  return (
    <AppShell>
      <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827' }}>Analyze Incoming Email</h1>
          <p style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>Paste email headers and body to run AI threat detection</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 20, alignItems: 'start' }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ borderBottom: '1px solid #E5E7EB' }}>
              {[
                { label: 'Sender:', field: 'sender', placeholder: 'suspicious@external.com' },
                { label: 'Recipient:', field: 'recipients', placeholder: 'you@company.com' },
            ...
            <div style={{ padding: '12px 24px', borderTop: '1px solid #F3F4F6', display: 'flex', gap: 12, background: '#F9FAFB' }}>
              ...
              >
                {scanning ? '⏳ Scanning...' : '🛡️ Analyze Email'}
              </button>
